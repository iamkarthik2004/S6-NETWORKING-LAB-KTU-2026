...
int main() {
    ...
    while (!disconnect) {
        sender(); // call sender
        for (k = 1; k <= 10000000; k++); // delay loop
...
void sender() {
    ...
    if (turn == 's') { // sender turn
        if (errorack == 0) { // Ack didn't arrive
            ...
            printf("SENDER: sent packet with seq NO:%d\n", p.seq);
            errorframe = rand() % 4;
            // Message below is printed only if Error occurred while sending Packet
            printf("%s\n", (errorframe == 0 ? "Error While sending Packet" : ""));
    ...
    if (turn == 'r') {
        if (errorframe != 0) {
            if (p.seq == frexp) { 
                ...
                // note sequence number of frame arrived to send acknowledgement
                ak = p.seq;
